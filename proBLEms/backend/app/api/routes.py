...
from app.models.schemas import SessionInfo, SessionConfig, \
    StopSessionRequest, SavePathRequest, ScannerData
...
def get_session_manager() -> SessionManager:
    from app.main import \
        session_manager  # ленивый импорт, чтобы избежать циклов
    return session_manager


@router.post("/session/start")
def start_session(cfg: SessionConfig, sm: SessionManager = Depends(
        get_session_manager)) -> Dict[str, Any]:
    return sm.start_session(cfg.model_dump())


@router.post("/session/stop")
async def stop_session(req: StopSessionRequest, sm: SessionManager = Depends(
        get_session_manager)) -> Dict[str, Any]:
...
# WebSocket: /api/ws — клиент сначала шлёт
    ...
    try:
        ...
        if data.get("type") != "subscribe" or "sessionId" not in data:
            await websocket.send_json({
                "type": "error",
                "message": "First message must be subscribe with sessionId"
            })
        ...
        if session_id:
            sm.remove_websocket_connection(websocket)
