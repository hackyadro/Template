...
class MQTTClient:
    ...
    def connect(self, host: str = "localhost", port: int = 1883, username: Optional[str] = None, password: Optional[str] = None, keepalive: int = 60):
    ...
    def _on_connect(self, client, userdata, flags, rc):
        if rc == 0:
            ...
            client.subscribe("indoor/scan/data")
            client.subscribe("indoor/control/#")
        else:
    ...
    def _handle_scan_data(self, data: Dict[str, Any]):
        # обязательные поля
        for f in ("sessionId", "scannerId", "beaconReadings"):
            if f not in data:
    ...
    def _handle_control(self, topic: str, data: Dict[str, Any]):
        ...
        if command == "start_session":
            ...
            if sid:
                # stop_session — асинхронный; ответ публиковать не будем, чтобы не блокировать
                import asyncio
                if self.session_manager.loop and not self.session_manager.loop.is_closed():
                    fut = asyncio.run_coroutine_threadsafe(self.session_manager.stop_session(sid), self.session_manager.loop)
                    try:
                        res = fut.result(timeout=3)
                        self.publish("indoor/control/stop_session/response", res)
                    except Exception as e:
                        self.publish("indoor/control/stop_session/response", {"status": "error", "message": str(e)})
