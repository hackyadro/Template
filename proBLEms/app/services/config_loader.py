...
class ConfigLoader:
                ...
                for row in reader:
                    if len(row) >= 3:
                        beacons.append({
                            ...
                            "x": float(row[1]),
                            "y": float(row[2]),
                            "z": float(row[3]) if len(row) > 3 else 1.5,
                            "txPower": -59
                        })
        return beacons
    ...
    def _parse_beacon_row(self, row: Dict[str, str]) -> Optional[Dict[str, Any]]:
        try:
            ...
            z_val = row.get("Z") or row.get("z")
            z = float(z_val) if z_val is not None else 1.5
            return {"id": str(name).strip(), "x": x, "y": y, "z": z, "txPower": -59}
